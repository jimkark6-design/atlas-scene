...
function cleanType(value: unknown) {
  ...
  return (SFX_TYPES as readonly string[]).includes(type) ? type : "NONE";
}
...
function normalizeEvents(events: any[], timeline: any[]) {
  ...
  for (const event of result) {
    ...
    if (
      ...
      Math.abs(previous.at - event.at) < 0.2
    ) {
      continue;
    }
...
- Every SFX must have a visible editorial reason.
...
- Avoid repetitive WHOOSH/IMPACT patterns.
...
WATER
Water spray / pressure-washing action.

FOAM
Foam / soap application.

WIPE
Microfiber / wiping / brushing / surface cleaning motion.

MACHINE
Machine / polisher / tool action.
...
CLICK
Button / contact / booking style accent.

POP
Small detail / reveal accent.
...
3. Is the timing tied to the action peak / reveal / cut?
